  ...
  for (const a of spec.agents) {
    ...
    files.push({
      ...
      content: JSON.stringify({
        ...
        note: "Create by hand on a sandbox number first. AGENT_CREATE is not automated: a new agent answers real callers.",
      }, null, 2) + "\n",
    });
  }
...
function buildDoc(
  ...
  return [
    ...
    "",
    "Nothing here has been applied anywhere. Each step reaches a live system, so each one is a person's decision.",
    "",
    "1. **Database.** Create a Supabase project for this business (not the template's), review `schema.sql`, then run it.",
    `2. **Config.** Copy \`vertical.ts\` to \`src/verticals/${spec.vertical.id}.ts\` in a new deployment and point \`src/verticals/active.ts\` at it. \`npx tsc --noEmit\` then checks the engines against the new states and actions.`,
    "3. **Extraction.** Rewrite the call and mail extraction schemas for the new tables (see NEEDS_PERSON above).",
    `4. **Memory.** Set \`COGNEE_DATASET=${spec.memory.dataset}\`.`,
    "5. **Workflows.** Import each file in `n8n/` through n8n (Workflows → Import from File). On n8n Cloud, first replace `{{ $env.SHIPMATE_BASE }}` with the new deployment's URL — Cloud restricts `$env`, which is why `scripts/n8n-deploy.mjs` substitutes it for the template. Leave them switched off until step 6 is tested.",
    "6. **Agents.** Create each agent in `agents/` by hand on a sandbox number, paste its prompt, attach its knowledge sources, set its webhook — then test before it answers a real line.",
    "",
  ].join("\n");
}
